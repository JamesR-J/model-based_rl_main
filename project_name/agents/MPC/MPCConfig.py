...
def get_MPC_config():
    ...
    config.BASE_NSAMPS = 30
    config.GAMMA = 1.25  # This is something else not discount value
    config.DISCOUNT_FACTOR = 1.0
    config.N_ELITES = 6  # TODO check it works for diff numbers of this
    config.iCEM_ITERS = 5
    config.XI = 0.3

    config.ACTIONS_PER_PLAN = 1  # How many actions to keep out of each total plan iteration, must be less than planning horizon
