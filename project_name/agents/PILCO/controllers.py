...
class LinearController(nn.Module):
    ...
    action_dim: int
    max_action: float
    w_init: Callable = nn.initializers.uniform(scale=1.0)
    b_init: Callable = nn.initializers.uniform(scale=1.0)
    ...
    def __call__(self, m, s):
       ...
       """
        ...
        M, S, V2 = self._squash_sin(M, S)
        V = V @ V2

        return M, S, V

    def _squash_sin(self, m, s):
        """
        ...
        """
        k = jnp.shape(m)[1]
        max_action = self.max_action * jnp.ones((1, k))

        M = max_action * jnp.exp(-0.5 * jnp.diag(s)) * jnp.sin(m)

        lq = -0.5 * (jnp.diag(s)[:, None] + jnp.diag(s)[None, :])
        q = jnp.exp(lq)
        S = (jnp.exp(lq + s) - q) * jnp.cos(m.T - m) - (jnp.exp(lq - s) - q) * jnp.cos(m.T + m)
