...
def squared_exponential_kern_fn(
    params: dict, x1: Array, x2: Array = None
) -> jnp.float64:
...
class Stationary(Kernel, abc.ABC):
    def __init__(
        self,
        lengthscales: Optional[Union[jnp.float64, Array]] = jnp.array(
...
class Rectangle(Stationary):
    def __init__(
        self,
        # width: Optional[Union[jnp.float64, Array[InputDim]]] = jnp.array(
        #     [1.0], dtype=default_float()
        # ),
        # height: Optional[jnp.float64] = 1.0,
        lengthscales: Optional[Union[jnp.float64]] = jnp.array(
            [1.0], dtype=default_float()
        ),
        variance: Optional[Union[jnp.float64]] = jnp.array(
            [1.0], dtype=default_float()
        ),
