...
from GPJax_AScannell.gpjax.custom_types import Input1, Input2, MultiOutputCovariance
from GPJax_AScannell.gpjax.kernels import Combination, Kernel
from GPJax_AScannell.gpjax.kernels.base import batched_covariance_map
from GPJax_AScannell.gpjax.utilities.ops import batched_diag, leading_transpose
...
def separate_independent_cov_fn(params: dict,
                                kernels: List[Kernel],
                                X1: Input1,
                                X2: Input2 = None,
                                full_cov: Optional[bool] = True,
                                full_output_cov: Optional[bool] = True) -> MultiOutputCovariance:
    Kxxs = jax.tree_map(lambda kern, params_: kern(params_, X1, X2, full_cov=full_cov), kernels, params)
...
class MultioutputKernel(Kernel):
    ...
    def __call__(
        ...
        if not full_cov:
            return self.K_diag(params, self.kernels, X1, full_output_cov=full_output_cov)
...
class SeparateIndependent(MultioutputKernel, Combination):
    ...
    def K(params: dict,
          ...
          X1: Input1,
          X2: Input2 = None,
          full_output_cov: Optional[bool] = False) -> MultiOutputCovariance:
        return separate_independent_cov_fn(params, kernels, X1, X2, full_cov=True, full_output_cov=full_output_cov)
        # return separate_independent_cov_fn(params, kernels, X1, X2, full_output_cov)

    @staticmethod
    def K_diag(params: dict,
               kernels: Union[List[Kernel], Kernel],
               X: Input1,
               full_output_cov: Optional[bool] = False) -> MultiOutputCovariance:
        return separate_independent_cov_fn(params, kernels, X1=X, full_cov=False, full_output_cov=full_output_cov)
